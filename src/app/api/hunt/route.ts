...
function computeScore(
  ...
  bd.activityRecency = Math.min(20, Math.min(12, Math.log10(Math.max(cnt90,1)+1)*6) + Math.min(5, repos365*1.0) + (ev180.length > ev90.length*1.2 ? 3 : 0));
  ...
  // Follower impact bonus (up to 11 points for absolute legends like Linus)
  const followerPts = Math.min(11, Math.log10(Math.max(user.followers || 0, 1)) * 2.5);
  pts += followerPts;

  bd.profileSignal = Math.min(20, Math.round(pts));
